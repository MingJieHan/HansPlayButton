...
@interface HansPlayButton ()<CAAnimationDelegate>
...
@implementation HansPlayButton
@synthesize transformanimationDuration;
@synthesize positionAnimationDuration;
@synthesize LineColor;

- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        transformanimationDuration = 0.5;
        positionAnimationDuration = 0.3f;
        LineColor = [UIColor colorWithRed:12/255.0 green:190/255.0 blue:6/255.0 alpha:1];
        [self setupUI];
    }
    return self;
}
...
#pragma mark -添加layer
- (void)setupUI {
    _buttonState = HansPlayButtonStatePause;
...
- (void)positiveLineAnimation {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(positionAnimationDuration/2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        self->_leftLineLayer.path = leftPath.CGPath;
        [self->_leftLineLayer addAnimation:[self pathAnimationWithDuration:self->positionAnimationDuration/2] forKey:nil];
        //右侧竖线缩放
        UIBezierPath *rightPath = [UIBezierPath bezierPath];
        [rightPath moveToPoint:CGPointMake(0.8*a, 0.8*a)];
        [rightPath addLineToPoint:CGPointMake(0.8*a, 0.2*a)];
        self->_rightLineLayer.path = rightPath.CGPath;
        [self->_rightLineLayer addAnimation:[self pathAnimationWithDuration:self->positionAnimationDuration/2] forKey:nil];
    });
}
...
- (void)positiveTransformAnimation {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(transformanimationDuration*0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self strokeEndAnimationFrom:1 to:0 onLayer:self->_leftLineLayer name:nil duration:self->transformanimationDuration/2 delegate:nil];
    });
}
...
- (void)inverseTransformAnimation {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(transformanimationDuration*0.75 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        //右侧竖线
        [self strokeEndAnimationFrom:0 to:1 onLayer:self->_rightLineLayer name:RightLineAnimationName duration:self->transformanimationDuration/4 delegate:self];
        //弧线
        [self strokeEndAnimationFrom:1 to:0 onLayer:self->_arcLayer name:nil duration:self->transformanimationDuration/4 delegate:nil];
    });
}
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    ...
    BOOL isTriangle = [name isEqualToString:TriangleAnimationName];
    BOOL isRightLine = [name isEqualToString:RightLineAnimationName];
    if (isRightLine && _buttonState == HansPlayButtonStatePlay) {
...
- (void)setButtonState:(HansPlayButtonState)buttonState {
    if (_isAnimating) {
        return;
    }
    _buttonState = buttonState;
    if (buttonState == HansPlayButtonStatePause) {
        //播放->暂停
        _isAnimating = YES;
    ...
    }   else if (buttonState == HansPlayButtonStatePlay) {
        //暂停->播放
        _isAnimating = YES;
        //竖线正向动画
